...
<html>
  ...
  <body>
    <h1>TEIガイドラインで定義されているモジュール・要素・属性の一覧</h1>
      <p>このページは、TEIガイドラインで定義されている<xsl:value-of select="count(distinct-values(/tei:TEI/tei:text/tei:body//tei:elementSpec/@ident))" />個の要素と<xsl:value-of select="count(distinct-values(/tei:TEI/tei:text/tei:body//tei:attDef/@ident))"/>個の属性を通覧できるようにしたものである。要素はガイドラインの記述に沿ってモジュール別に列挙した。末尾に要素名による索引を付している。属性は属性クラス別に列挙した。このページはガイドラインの付録リファレンスのソースファイルから、XSLTスタイルシートにより機械的に生成されたものである。</p>
